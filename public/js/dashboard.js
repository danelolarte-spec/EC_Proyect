async function DashboardView(root) {
  ...
  const stats = UI.el('div', { class: 'cards-grid' }, [
    statCard('Proyectos activos', data.active, ''),
    statCard('Proyectos completados', data.completed, 'ok'),
    statCard('Total proyectos', data.total, ''),
    statCard('Tareas vencidas', data.overdueTasks, data.overdueTasks ? 'danger' : 'ok'),
    statCard('Tareas pendientes', data.pendingTasks, 'warn'),
    statCard('Usuarios', data.totalUsers, ''),
    statCard('Áreas', data.totalAreas, ''),
    statCard('Contenido del mes', data.contentThisMonth, 'warn')
  ]);
...
function statCard(label, num, variant) {
  return UI.el('div', { class: 'stat-card ' + (variant || '') }, [
    UI.el('h3', {}, label),
    UI.el('div', { class: 'num' }, String(num || 0))
  ]);
}
